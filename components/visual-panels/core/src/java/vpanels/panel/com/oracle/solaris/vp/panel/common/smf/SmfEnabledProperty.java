/*
 * CDDL HEADER START
 *
 * The contents of this file are subject to the terms of the
 * Common Development and Distribution License (the "License").
 * You may not use this file except in compliance with the License.
 *
 * You can obtain a copy of the license at usr/src/OPENSOLARIS.LICENSE
 * or http://www.opensolaris.org/os/licensing.
 * See the License for the specific language governing permissions
 * and limitations under the License.
 *
 * When distributing Covered Code, include this CDDL HEADER in each
 * file and include the License file at usr/src/OPENSOLARIS.LICENSE.
 * If applicable, add the following below this CDDL HEADER, with the
 * fields enclosed by brackets "[]" replaced with your own identifying
 * information: Portions Copyright [yyyy] [name of copyright owner]
 *
 * CDDL HEADER END
 */

/*
 * Copyright (c) 2009, 2012, Oracle and/or its affiliates. All rights reserved.
 */

package com.oracle.solaris.vp.panel.common.smf;

import com.oracle.solaris.scf.common.ScfException;
import com.oracle.solaris.vp.util.misc.property.BooleanProperty;

/**
 * The {@code SmfEnabledProperty} class is an {@link SmfMutableProperty} that
 * represents the enabled state of a service instance in the SMF repository.
 */
public class SmfEnabledProperty extends BooleanProperty
    implements SmfMutableProperty<Boolean> {

    //
    // Static data
    //

    /**
     * The default name of this property ("enabled") if none is specified.
     */
    public static final String NAME = "enabled";

    //
    // Instance data
    //

    private HasService info;

    //
    // Constructors
    //

    /**
     * Constructs a {@code SmfEnabledProperty}.
     *
     * @param	    name
     *		    the name of this property
     *
     * @param	    info
     *		    source of the service
     */
    public SmfEnabledProperty(String name, HasService info) {
	super(name);
	this.info = info;
    }

    /**
     * Constructs a {@code SmfEnabledProperty} with a {@link #NAME default
     * name}.
     *
     * @param	    info
     *		    source of the service
     */
    public SmfEnabledProperty(HasService info) {
	this(NAME, info);
    }

    //
    // SmfMutableProperty methods
    //

    @Override
    public Boolean getRepoValue() throws ScfException {
	return info.getService().isPersistentlyEnabled();
    }

    @Override
    public void saveToRepo() throws ScfException {
	Boolean value = getValue();
	setRepoValue(value);
	save();
    }

    @Override
    public void setRepoValue(Boolean value) throws ScfException {
	ServiceMXBean service = info.getService();
	service.setPersistentlyEnabled(value);
    }

    @Override
    public void updateFromRepo(boolean force) throws ScfException {
	update(getRepoValue(), force);
    }
}
