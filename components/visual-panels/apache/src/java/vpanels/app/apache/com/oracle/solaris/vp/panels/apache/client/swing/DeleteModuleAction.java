...
public class DeleteModuleAction
    extends DeleteManagedObjectAction<Module, Object, List<Module>> {
    ...
    public DeleteModuleAction(SwingControl control) {
    ...
    @Override
    public Object getRuntimeInput(List<Module> selection, Object input)
	...
	String message = Finder.getString(
	    "modules.action.delete.confirm." + count,
    ...
    @Override
    public List<Module> workBusy(List<Module> selection, Object input)
	throws ActionAbortedException, ActionFailedException,
	ActionUnauthorizedException {

	if (!selection.isEmpty()) {
	    Modules modules = selection.get(0).getParent();

	    for (Module module : selection) {
		modules.scheduleRemove(module);
	    }

	    // If the running Control is a ModuleControl for a just-deleted
	    // Module, reset the Control and navigate back to its parent
	    Control child = control.getRunningChild();
	    if (child instanceof ModuleControl) {
